...
def test_submit_upgrade_request_persists_optional_transaction_reference():
    token = real_auth_service.create_access_token(TENANT_ID, "customer@example.com")
    captured = {}
    ...
    with patch("src.api.routes.upgrade_requests.supabase_insert", side_effect=fake_insert):
        resp = client.post("/api/v2/upgrade-requests",
                            json={"name": "Jane", "email": "jane@example.com", "plan": "growth",
                                  "transaction_reference": "TXN-88221-UBL"},
                            headers={"Authorization": f"Bearer {token}"})

    assert resp.status_code == 200, resp.text
    assert captured['data']['transaction_reference'] == "TXN-88221-UBL"


def test_submit_upgrade_request_transaction_reference_defaults_to_none():
    token = real_auth_service.create_access_token(TENANT_ID, "customer@example.com")
    captured = {}

    def fake_insert(table, data):
        captured['data'] = data
        return {"id": "req-3", **data}
    ...
    assert resp.status_code == 200, resp.text
    assert captured['data']['transaction_reference'] is None
...
def test_activate_upgrade_request_stamps_plan_activated_at_for_paid_plans():
    """growth is a paid plan (expires 30 days after activation, see
    plan_service.PAID_PLANS) — activating it must set plan_activated_at so
    that expiry has something to count from."""
    admin_token = real_auth_service.create_access_token("admin-tenant-id-2", ADMIN_EMAIL)
    updates = []

    def fake_select(table, params=None):
        if table == "upgrade_requests":
            return [{"id": "req-2", "tenant_id": TENANT_ID, "requested_plan": "growth", "status": "pending"}]
        return []
    ...
    with patch("src.api.routes.platform_admin.is_super_admin", side_effect=lambda e: (e or "").lower() == ADMIN_EMAIL), \
         patch("src.api.routes.platform_admin.supabase_select", side_effect=fake_select), \
         patch("src.lib.supabase_client.supabase_update", side_effect=fake_update):
        resp = client.post("/api/v2/admin/upgrade-requests/req-2/activate",
                            headers={"Authorization": f"Bearer {admin_token}"})

    assert resp.status_code == 200, resp.text
    tenant_update = next(u for u in updates if u[0] == "tenants")
    assert tenant_update[2].get("plan_activated_at") is not None


def test_activate_upgrade_request_does_not_stamp_activation_date_for_free_plan():
    admin_token = real_auth_service.create_access_token("admin-tenant-id-3", ADMIN_EMAIL)
    updates = []

    def fake_select(table, params=None):
        if table == "upgrade_requests":
            return [{"id": "req-3", "tenant_id": TENANT_ID, "requested_plan": "free", "status": "pending"}]
        return []
    ...
    with patch("src.api.routes.platform_admin.is_super_admin", side_effect=lambda e: (e or "").lower() == ADMIN_EMAIL), \
         patch("src.api.routes.platform_admin.supabase_select", side_effect=fake_select), \
         patch("src.lib.supabase_client.supabase_update", side_effect=fake_update):
        resp = client.post("/api/v2/admin/upgrade-requests/req-3/activate",
                            headers={"Authorization": f"Bearer {admin_token}"})

    assert resp.status_code == 200, resp.text
    tenant_update = next(u for u in updates if u[0] == "tenants")
    assert "plan_activated_at" not in tenant_update[2]
