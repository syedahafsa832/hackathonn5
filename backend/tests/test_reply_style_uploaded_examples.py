"""
Superseded: this file previously documented "any uploaded example present
skips the volume gate entirely" as an intentional fix. That behavior itself
turned out to be the bug — it let a single hand-written example make
"Learned Style is ready" appear with 0 approved replies, directly
contradicting the "0 of 20 approved replies" counter shown right next to it
on the Settings page.

Corrected rule: eligibility is always decided by real approved-reply volume
(MIN_APPROVED_REPLIES_TO_LEARN), matching the fixed "X of 20" the UI states.
Uploaded examples only ever enrich the extraction prompt once eligible via
approved replies; they never lower or bypass the threshold. use_uploaded_only
still controls *content* (exclude ticket-history text from the prompt), not
eligibility.
"""
import os
import sys
...
def test_uploaded_example_alone_does_not_bypass_the_approved_reply_gate():
    """The reported bug: a fresh example, 0 approved replies, must NOT
    generate a learned profile (that made "ready to switch" appear next to
    a "0 of 20" counter)."""
...
def test_regenerate_if_due_does_not_trigger_from_an_uploaded_example_alone():
    """The opportunistic auto-trigger (checked on Settings page load) must
    NOT fire off an uploaded example with 0 approved replies - only real
    approved-reply volume crossing the threshold may trigger the first
    profile generation."""
