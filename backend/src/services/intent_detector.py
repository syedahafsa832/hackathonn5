...
- "return" — customer wants to send an item they already RECEIVED back for a refund, OR is asking whether THEIR OWN order/item is eligible to be returned / still within the return window (a return-eligibility question about a specific order is "return", not "refund" and not "none" — the customer wants to know about a return, not money back for some unrelated reason, and this is not a general policy question since it's about their own order). Key phrases: "return this", "send it back", "want to return", "return my order", "return item X", "how do I return", "changed my mind" (about a received item), "don't want this anymore" (about a received item), "am I eligible to return order #X", "is my order still within the return window", "can I still return #X". Does NOT include wanting a different size/color/product instead — that is "exchange".
...
- "refund" — customer wants money back for a reason OTHER than sending a physical item back (e.g. billed incorrectly, price adjustment, item damaged/wrong and no replacement wanted). If the customer talks about sending something back, or asks whether their order is eligible to be returned, use "return" instead — never "refund" just because a return would eventually result in money back. If the customer wants a different size/color/product, use "exchange" instead.
- "none" — general question, tracking inquiry, product question, or a POLICY question about how returns/exchanges/refunds/cancellations work IN GENERAL, with no specific order of the customer's own in question (e.g. "what is your return policy", "what is your cancellation policy", "how long do I have to return something", "do you offer exchanges") — asking ABOUT a policy in the abstract is never an action request, even if it uses the words return/exchange/refund/cancel. The moment the customer asks about THEIR OWN order (names an order number, says "my order", "this order", "the one I got"), it is no longer a general policy question — classify by what they're asking about that order (e.g. "is order #1009 still within the return window?" is "return", not "none"). A past-tense statement or question about a cancellation that already happened ("why was my order cancelled?", "my order was cancelled") or a hypothetical ("what happens if I cancel?") is also "none" — the customer is not asking to cancel anything right now. Only classify as return/exchange/refund/cancel when the customer is asking to DO something to, or asking a return-eligibility question about, their own order right now.
...
- "Is order #1009 still within the return window?" -> {{"action_type": "return", "order_id": "1009", ...}}
- "I'd like to return order #1009. Can you check if I'm eligible?" -> {{"action_type": "return", "order_id": "1009", ...}} (the customer said "return", not "refund" - classify by their actual word)
